...
| Table                            | Purpose                              | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `companies` (new columns)        | Structured profile.                  | Identity: trading name, contact email/phone/address, registration number, tax identifier. Business: products, revenue model, secondary objectives, sales/marketing channels. Brand: personality, voice, visual guidance, approved/prohibited phrases, claims allowed / requiring evidence. Compliance: jurisdictions, regulators, legal disclaimers, data handling rules.                                                                                                                                                                                                                   |
| `company_ai_policies`            | AI operations policy (1:1).          | Allowed providers, default research limit, deep-research / external-action / browser / auto-send modes (`disabled`, `approval_required`, `allowed`), stale-knowledge policy (`exclude` / `mark_stale`), custom AI rules.                                                                                                                                                                                                                                                                                                                                                                    |
| `knowledge_items`                | Knowledge library (one row/version). | Scope company/GLOBAL (`company_id` NULL ⇔ GLOBAL, check constraint), department, title, summary, content, type, category, tags, provenance (source type/reference/URL/file ref/owner/notes), confidence, verification, status, sensitivity, `usable_as_unverified`, effective/review/expiry/last-verified dates, `conflict_key`, `version`, `lineage_id`, supersedes/superseded-by, created/updated/approved by, generated `search_vector` (GIN). Partial unique indexes: one APPROVED and one open version per lineage. AI sources can never be `management_confirmed` (check constraint). |
| `knowledge_links`                | Explicit links.                      | Knowledge ↔ task or agent (exactly one, check constraint).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `brand_rules`                    | Brand rules.                         | Category, channel, severity (info/required/critical), active, status (draft/approved/archived), approver. `company_id` NULL = global.                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `commercial_rules`               | Commercial rules.                    | Category, `applies_to` action key, effect (info/limit/require_approval/prohibit), limit amount, currency, period, required approval permission.                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `compliance_rules`               | Compliance rules.                    | Action pattern, jurisdiction, effect (info/require_approval/prohibit/require_disclosure), disclosure text, required permission.                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `knowledge_access_policies`      | Agent knowledge access.              | Company + (agent \| department \| all agents) → max sensitivity (confidential/restricted). Default without rows: PUBLIC + INTERNAL.                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `agent_knowledge_profiles`       | Per-agent knowledge defaults.        | Additions to the template profile: required types, preferred tags, always-include brand/commercial rule categories.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `tasks.allow_unverified_context` | Task flag.                           | The task may use clearly-labelled UNVERIFIED research.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |

Stage 03 audit actions: `company.profile_updated`, `company.ai_policy_updated`,
`brand_rule.created|updated|approved|archived|restored` (same for
`commercial_rule` and `compliance_rule`), `knowledge.created`,
`knowledge.updated`, `knowledge.version_created`, `knowledge.submitted`,
`knowledge.approved`, `knowledge.rejected`, `knowledge.superseded`,
`knowledge.archived`, `knowledge.linked`, `knowledge.unlinked`,
`knowledge_access.granted|revoked`, `agent.knowledge_profile_changed`,
`context.preview_generated`, `security.knowledge_access_denied`,
...
- **Knowledge** (`packages/shared/src/knowledge.ts`): 25 types; status draft, review, approved, superseded, archived; confidence low/medium/high; verification unverified, partially_verified, verified, management_confirmed; 13 source types; sensitivity public, internal, confidential, restricted; rule severity info, required, critical; rule status draft, approved, archived
...
- **Development seed** (`src/seed/dev/`): the three companies (upserted by
  slug; Stage 03 profiles, AI policies, 17 knowledge items incl. 2 GLOBAL, 10 rules incl.
  Opportunitygrad's ₹100 Meta budget limit, links and one access policy —
  known high-level facts only), 6 development user accounts, 18 agents (with template grants), 13 tasks (incl. a 4-level hierarchy), 5 approvals,
